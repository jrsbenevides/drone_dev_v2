...
class globalPlanner:
    ...
    def callbackOdom(self, odom):
        ...
        if self.viconOne == 0:
            ...
            if self.usingOrbs == 0:
                ...
                self.curr_pos["x"] = posicao[0] + 0.75
                self.curr_pos["y"] = posicao[1] + 1
                self.curr_pos["z"] = posicao[2] #+ 0.5
    ...
    def callbackVisao(self, target):
        ...
        if self.startSM == 1 and (self.time() - self.tt > 62) and self.rotas["x"] != 90:
        ...
        if self.startSM == 1 and self.ROTINA_DIEGO >= 1 and (self.ROTINA_ORBS + self.PERDEU_ORBS) == 0:
            ...
            if status == self.abortei:
                ...
                if self.ROTINA_DIEGO == 62:
                    if (self.targetPixel["x"] - self.pixelsPouso) < self.pixelsPouso and (self.targetPixel["y"] - self.pixelsPouso) < self.pixelsPouso: # dentro do range
