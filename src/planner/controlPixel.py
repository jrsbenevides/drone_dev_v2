...
class globalPlanner:
    ...
    def callbackVisao(self, target):
        ...
        if self.startSM == 1 and (self.time() - self.tt > 1):  #ALTERAR URGENTE..SOLUCAO TEMPORARIA!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            ...
            if self.ROTINA_DIEGO == 20:
                ...
                if len(a[0]) != 0:
                    ...
                    if self.uniqueBaseBaixo == 0:
                        ...
                        self.goToLocal(flag=self.desFinal)
                        print("rotina de aproximcao")

        
                    for pose in target.poses:
                        self.u[0]    = (pose.position.x)
                        self.v[0]    = (pose.position.y)
                        self.area[0] = (pose.position.z)
                        self.yaw[0]  = (pose.orientation.y)
                        self.m[0]    = (pose.orientation.z)
                        self.flag[0] = (pose.orientation.w)
        ...
        if self.startSM == 1 and self.ROTINA_DIEGO >= 1 and (self.ROTINA_ORBS + self.PERDEU_ORBS) == 0:
            ...
            if status == self.abortei:
                ...
                if self.estado != 2:
                    self.estado = 2
                    print("ocupado")
                if self.autorizaPub == self.calibraAlvo:
                    self.goToLocal(x=self.u[0],y=self.v[0],flag = self.calibraAlvo)
                ...
                if self.ROTINA_DIEGO == 60:
                    print("pronto para aproximar com PID")
                    self.goToLocal(x=self.u[0],y=self.v[0],flag = self.calibraAlvo)
                    self.autorizaPub = self.calibraAlvo
