...
class globalPlanner:
    def __init__(self):
        ...
        self.alturaDrone = 1
        self.rotas = {"x": [0, 1, 0, -1, 0, -90], "y": [0, 0, -1, 0, 1, -90], "z": [self.alturaDrone, self.alturaDrone, self.alturaDrone, self.alturaDrone, self.alturaDrone, -90], "yaw": [0, 0, 0, 0, 0, 0]}
        self.pos = 1
        self.ROTINA_DIEGO = 0
    ...
    def goToGlobal(self, x=-100, y=-100, z=-100, yaw=0, t=0, flag=10):
        x1 = x - self.curr_pos["x"] if x != -100 else self.curr_pos["x"]
        y1 = y - self.curr_pos["y"] if y != -100 else self.curr_pos["y"]
        z1 = z - self.curr_pos["z"] if z != -100 else self.curr_pos["z"]
    ...
    def callbackOdom(self, odom):
        self.posicaoAtual["odom"]["x"] = odom.pose.pose.position.x + 0.75
        self.posicaoAtual["odom"]["y"] = odom.pose.pose.position.y + 1
        self.posicaoAtual["odom"]["z"] = odom.pose.pose.position.z #+ 0.5
        self.posicaoAtual["odom"]["yaw"] = odom.pose.pose.orientation.z 

        if self.usingOrbs == 0:
            self.curr_pos["x"] = odom.pose.pose.position.x + 0.75
            self.curr_pos["y"] = odom.pose.pose.position.y + 1
            self.curr_pos["z"] = odom.pose.pose.position.z #+ 0.5
    ...
    def callbackVisao(self, target):
        ...
        if self.startSM == 1 and self.estado > 1:
        ...
        if self.startSM == 1 and self.ROTINA_DIEGO >= 1 and (self.ROTINA_ORBS + self.PERDEU_ORBS) == 0:
            ...
            if status == self.abortei:
                ...
                if self.ROTINA_DIEGO == 50:
                    ...
                    self.goToGlobal(x=self.rotas["x"][self.pos-1], y=self.rotas["x"][self.pos-1])
                ...
                #         # else:
                #         sub = 0
                #         # self.targetPixel["x"] = (np.mean(self.u)+184.85) * (self.curr_pos["z"] - sub) / self.focoCamera["x"] - 0.0879
                #         # self.targetPixel["y"] = (np.mean(self.v)-30.15) * (self.curr_pos["z"] - sub) / self.focoCamera["y"] - 0.1066
                #         self.targetPixel["x"] = np.mean(self.u) * (self.curr_pos["z"] - sub) / self.focoCamera["x"]
                #         self.targetPixel["y"] = np.mean(self.v) * (self.curr_pos["z"] - sub) / self.focoCamera["y"]
        ...
        if self.startSM == 1 and (self.ROTINA_ORBS + self.PERDEU_ORBS + self.ROTINA_DIEGO) == 0:
            if status == self.idle: # Idle
                if self.estado == 0:
                    ...
                    self.goToLocal(z=self.rotas["z"][0])
                    #self.desce.publish(Empty())
