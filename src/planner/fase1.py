...
class globalPlanner:
    def __init__(self):
        ...
        self.rotas = {"x": [0, 3.5, 0, -90], "y": [0, 0.5, 0, -90], "z": [self.alturaDrone, self.alturaDrone, self.alturaDrone, -90], "yaw": [0, 0, 0, 0, 0]}
        self.pos = 1
        self.ROTINA_DIEGO = 0
    ...
    def goToGlobal(self, x=0, y=0, z=0, yaw=0, t=0, flag=10):
        x1 = x - self.curr_pos["x"] - 0.75
        y1 = y - self.curr_pos["y"] - 1
        z1 = z - self.curr_pos["z"] #- 0.5
        ...
        if self.startSM == 1 and self.ROTINA_DIEGO >= 1 and (self.ROTINA_ORBS + self.PERDEU_ORBS) == 0:
            ...
            if status == self.abortei:
                ...
                if self.ROTINA_DIEGO == 20 or self.ROTINA_DIEGO == 21:
                    if self.estado == 200:
                        self.goToGlobal(x=curr_pos["x"],y=self.curr_pos["y"],z=self.alturaDrone)
                        self.estado = 201
                    else:
                        self.pos += 1 
                ...
                if self.estado == 0:
                    ...
                    if(self.time() - self.timeStop > self.tempoParar):
                        ...
                        if self.ROTACIONAR == 0: # Caso tenha chego em um waypoint mas ainda nao rotacionou
                            ...
                            self.goToGlobal(x=self.rotas["x"][self.pos], y=self.rotas["y"][self.pos],z=self.curr_pos["z"])
                            self.pos += 1
                            self.ROTACIONAR = 0
                            self.estado = 10
