...
import detector_frontal
import pouso_inferior
...
class image_converter:
  ...
  def __init__(self):
    ...
    self.caminf = cv2.VideoCapture("http://192.168.4.1:8000/stream.mjpg")
  ...
  def callback(self,data):
    ...
    if self.flag_boot:
      ...
      center_rotation = ()
      command = 0
      if self.camera_control_angle != 0.0:
        try:
        ...
        cv_image,filtered,center,center_rotation = detector_frontal.landing_detector_main(cv_image,self.camera_control_angle,self.bebop_z)
        command = 120
        # filename = "image" + str(rospy.get_rostime().secs - rostime_initial) + ".jpg"
        # cv2.imwrite(filename, cv_image)

      else:
        ret, cv_image = self.caminf.read()
        if ret:
          cv_image,filtered,center,center_rotation = pouso_inferior.landing_detector_main(cv_image,self.camera_control_angle,self.bebop_z)
          command = 140

      
      if center:
        msg = self.build_PoseArray(center,center_rotation,command)
  ...
  def build_PoseArray(self,center,center_rotation,command):
    msg = PoseArray()
    for point in center:
      current_point = Point(float(242 - point[1]),float(302 - point[0]),float(point[2]))
      current_quaternion  = Quaternion(center_rotation[0][0],center_rotation[0][1],center_rotation[0][2],command)
