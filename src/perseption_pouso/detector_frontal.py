...
def carmona_epic_fun(cv_image):
	...
	blue_imgage = cv2.dilate(blueThres, kernel, iterations = 1)
	blue_imgage = cv2.erode(blueThres, kernel, iterations = 1)
...
def get_contours(cv_image,filtered,camera_control_angle,bebop_z):
	...
	for point in center:
		cv_image = cv2.line(cv_image, (point[0],240), (point[0],point[1]), (255,0,0), 2) # X
		cv_image = cv2.line(cv_image, (427,240), (point[0],240), (0,0,255), 2) # Y
		cv_image = cv2.line(cv_image, (427,240), (point[0],point[1]), (0,255,0), 2) # XY
	return(cv_image,filtered,center,center_rotation)
...
def get_rotation(center,camera_control_angle,bebop_z): # Need hardcode Kparam
    ...
    Kparam = [537.292878, 0.000000, 427.331854, 0.000000, 527.000348, 240.226888, 0.000000, 0.000000, 1.000000] # Bebop 2
